...
const privateUtils = {
      ...
      enter(path) {
        ...
        if (privateUtils.isFunctionNode(node)) {
          ...
          if (cursor && positionIn(node.loc, cursor, doc)) {
            newContext.smartSuggestions = privateUtils.getSmartSuggestionsAtCursor(ast, cursor, doc);
  ...
  getSmartSuggestionsAtCursor(ast: t.File, cursor: vscode.Position, doc: vscode.TextDocument): string[] {
