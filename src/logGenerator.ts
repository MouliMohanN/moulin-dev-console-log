...
export class LogGeneratorService {
  ...
  private buildLogObject(context: CodeContext, logItemsConfig: string[]): string {
    const parts: string[] = [];
    ...
    if (context.parentContext) {
      parts.push(`parent: ${this.buildLogObject(context.parentContext, logItemsConfig)}`);
    }

    return `{ ${parts.join(', ')} }`;
  }

  public generateConsoleLog(ctx: CodeContext, fileName: string, selectedItems?: string[]): string {
