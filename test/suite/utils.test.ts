...
import { extractVariableNames, positionIn } from '../../src/utils';
  ...
  suite('extractVariableNames', () => {
    ...
    test('should extract names from ObjectPattern', () => {
      const id = {
        type: 'ObjectPattern',
        properties: [
          { type: 'ObjectProperty', value: { type: 'Identifier', name: 'x' } },
          { type: 'ObjectProperty', value: { type: 'Identifier', name: 'y' } },
        ],
      };
    ...
    test('should handle nested patterns (simplified)', () => {
      const id = {
        ...
        elements: [
          { type: 'Identifier', name: 'a' },
          { type: 'ObjectPattern', properties: [{ type: 'ObjectProperty', value: { type: 'Identifier', name: 'b' } }] },
        ],
      };
      assert.deepStrictEqual(extractVariableNames(id), ['a', 'b']);
    });
